...
const shopPlaceholderSeed = shopName
  .split("-")
...
const isEditorRequest =
  window.Shopify.visualPreviewMode || window.Shopify.designMode;
...
class PlaceholderImage extends HTMLElement {
  connectedCallback() {
    ...
    if (cached != null && Theme.placeholders[type][cached]) {
      ...
      imageElement.alt =
        Theme.translations.placeholder_image || "Placeholder Image";
    ...
    const sectionIndex = sectionElement
      ? Array.from(sectionElement.parentElement?.children ?? []).indexOf(
          sectionElement
        )
      : -1;
