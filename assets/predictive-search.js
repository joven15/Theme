...
class PredictiveSearchComponent extends Component {
  ...
  #resizeObserver = new ResizeObserver((entries) => {
    for (const entry of entries) {
      this.style.setProperty(
        "--predictive-search-results-height",
  ...
  connectedCallback() {
    ...
    if (dialog) {
      ...
      document.addEventListener(ThemeEvents.megaMenuHover, this.#blurSearch, {
        signal,
      });
    }
  ...
  #getRecentlyViewed = async () => {
    ...
    try {
      ...
      const parsedNextPage = new DOMParser().parseFromString(
        resultsMarkup,
        "text/html"
      );
      const recentlyViewedProductsHtml = parsedNextPage.getElementById(
      ...
      const collectionElement = predictiveSearchResults.querySelector(
        "#predictive-search-products"
      );
